...
struct RenderPass {
    ...
    static func background(color: SIMD3<Float>, enabled: Bool) -> RenderPass {
        RenderPass(
            name: "Background",
            color: color,
            renderMask: RenderingConstants.backgroundRenderMask,
    ...
    static func foreground(color: SIMD3<Float>, enabled: Bool) -> RenderPass {
        RenderPass(
            name: "Foreground",
            color: color,
            renderMask: RenderingConstants.foregroundRenderMask,
...
protocol MetalRenderer {
    func render(
        ...
        aspect: Float,
        splatColor: SIMD3<Float>,
        influenceThreshold: Float
    )
}
...
protocol MetalOverlayStateManager {
    ...
    func shouldUpdateMetalDotsBuffer() -> Bool
}
...
final class DefaultMetalRenderer: MetalRenderer {
    ...
    func render(
        ...
        var splatColorVar = splatColor
        encoder.setFragmentBytes(&splatColorVar, length: MemoryLayout<SIMD3<Float>>.stride, index: 3)
        encoder.setFragmentBuffer(renderMaskBuffer, offset: 0, index: 4)
        var influenceThresholdVar = influenceThreshold
        encoder.setFragmentBytes(&influenceThresholdVar, length: MemoryLayout<Float>.stride, index: 5)
...
final class DefaultMetalOverlayStateManager: MetalOverlayStateManager {
    var renderMask: [Bool] = RenderingConstants.defaultRenderMask
    ...
    private var cachedMetalDotsHash: Int = -1
    private var needsMetalDotsBufferUpdate: Bool = true
    ...
    func updateMetalDots(_ dots: [MetalDot]) -> Bool {
        ...
        if metalDotsHash != newHash {
            ...
            needsMetalDotsBufferUpdate = true
            return true
        }
        return false
    }
    ...
    func shouldUpdateMetalDotsBuffer() -> Bool {
        if needsMetalDotsBufferUpdate || metalDotsHash != cachedMetalDotsHash {
            cachedMetalDotsHash = metalDotsHash
            needsMetalDotsBufferUpdate = false
            return true
        }
        return false
    }
...
final class DefaultMetalBufferManager: MetalBufferManager {
    ...
    func updateBuffers(with stateManager: MetalOverlayStateManager) {
        ...
        // Check if we need any updates first
        let needsRenderMaskUpdate = stateManager.shouldUpdateRenderMaskBuffer()
        let needsMetalDotsUpdate = stateManager.shouldUpdateMetalDotsBuffer()
        
        if needsRenderMaskUpdate || needsMetalDotsUpdate {
            startTime = PerformanceMonitor.startBufferUpdate()
        } else {
            startTime = nil // No work to do
        }
        
        // Update render mask buffer only if changed
        if needsRenderMaskUpdate {
        ...
        // Update unified MetalDot buffer only if changed (major optimization)
        if needsMetalDotsUpdate {
        ...
        // Only measure time if we actually did work
        if updatedAny {
            PerformanceMonitor.endBufferUpdate(startTime)
        }
    }
}

/// Centralized Metal service coordinator
final class MetalService {
    static let shared: MetalService = {
        do {
            return try MetalService()
        } catch {
            // Fallback to a no-op service for graceful degradation
            print("Metal initialization failed: \(error)")
            fatalError("Metal is required for this application")
        }
    }()
    ...
    private init() throws {
        self.deviceProvider = try DefaultMetalDeviceProvider()
        self.shaderCompiler = DefaultMetalShaderCompiler(device: deviceProvider.device)
        self.pipelineProvider = DefaultMetalPipelineProvider(device: deviceProvider.device)
        self.commandProvider = try DefaultMetalCommandProvider(device: deviceProvider.device)
        
        let shaders = try shaderCompiler.compileSplatterShader()
        self.pipelineState = try pipelineProvider.createSplatterPipeline(vertex: shaders.vertex, fragment: shaders.fragment)
        self.renderer = DefaultMetalRenderer(pipelineState: pipelineState, commandQueue: commandProvider.commandQueue)
    }
...
struct SplatterShaderSource {
    ...
    fragment float4 fragment_main(VertexOut in [[stage_in]],
                                  ...
                                  constant float3& splatColor [[buffer(3)]],
                                  constant bool* renderMask [[buffer(4)]],
                                  constant float& influenceThreshold [[buffer(5)]]) {
        float2 uv = in.uv;
        float field = 0.0;
        for (uint i = 0; i < SPLAT_DOT_LIMIT; ++i) {
            float radius = dots[i].radius;
            // Early exit for zero-radius dots (major optimization)
            ...
            if (dotType >= 0 && dotType < \(RenderingConstants.dotTypeCount) && renderMask[dotType]) {
                ...
                // Spatial culling: skip dots too far to influence this pixel
                float2 aspect_uv = float2((uv.x - center.x) * aspect, uv.y - center.y);
                float distSq = dot(aspect_uv, aspect_uv);
                
                // Quick distance check: if further than max influence radius, skip
                float maxInfluenceRadiusSq = \(RenderingConstants.maxInfluenceRadius) * \(RenderingConstants.maxInfluenceRadius);
                if (distSq > maxInfluenceRadiusSq) continue;
                
                // More precise culling: check if dot can actually contribute
                float radiusSq = radius * radius;
                float influence = radiusSq / (distSq + \(RenderingConstants.metalDistanceEpsilon));
                if (influence < influenceThreshold) continue; // Skip dots with negligible influence
        ...
        return float4(splatColor, alpha);
    }
    """
}
...
enum RenderingConstants {
    ...
    // Render mask constants: [central, large, medium, small, splash]
    static let backgroundRenderMask: [Bool] = [true, true, true, true, false]
    static let foregroundRenderMask: [Bool] = [true, true, true, false, false]
    static let defaultRenderMask: [Bool] = [true, true, true, true, false]
    
    // Performance monitoring
    static let enablePerformanceLogging = true
    
    // Spatial culling optimization
    static let enableSpatialCulling = true
    static let maxInfluenceRadius: Float = 0.2 // Max normalized radius for early culling
    static let minInfluenceThreshold: Float = 0.01 // Skip dots with less influence than this
}
...
final class PerformanceMonitor {
    ...
    private var bufferUpdateTimes: [Double] = []
    private let maxSamples = 60 // Keep last 60 samples
    
    // Display link for actual frame rate
    private var displayLink: CADisplayLink?
    private var lastFrameTime: CFTimeInterval = 0
    private var actualFrameTimes: [Double] = []
    private var frameDropCount = 0
    
    private static let shared = PerformanceMonitor()
    
    static func startRender() -> CFAbsoluteTime? {
        guard RenderingConstants.enablePerformanceLogging else { return nil }
        return CFAbsoluteTimeGetCurrent()
    }
    
    static func endRender(_ startTime: CFAbsoluteTime?) {
        guard RenderingConstants.enablePerformanceLogging, let start = startTime else { return }
        let duration = (CFAbsoluteTimeGetCurrent() - start) * 1000 // ms
        shared.addRenderTime(duration)
        // Immediate feedback for first few renders
        if shared.renderTimes.count <= 5 {
            print("[PERF] Frame \(shared.renderTimes.count): \(String(format: "%.2f", duration))ms")
            fflush(stdout)
        }
    }
    
    static func startUpdate() -> CFAbsoluteTime? {
        guard RenderingConstants.enablePerformanceLogging else { return nil }
        return CFAbsoluteTimeGetCurrent()
    }
    
    static func endUpdate(_ startTime: CFAbsoluteTime?) {
        guard RenderingConstants.enablePerformanceLogging, let start = startTime else { return }
        let duration = (CFAbsoluteTimeGetCurrent() - start) * 1000 // ms
        shared.addUpdateTime(duration)
    }
    
    static func startBufferUpdate() -> CFAbsoluteTime? {
        guard RenderingConstants.enablePerformanceLogging else { return nil }
        return CFAbsoluteTimeGetCurrent()
    }
    
    static func endBufferUpdate(_ startTime: CFAbsoluteTime?) {
        guard RenderingConstants.enablePerformanceLogging, let start = startTime else { return }
        let duration = (CFAbsoluteTimeGetCurrent() - start) * 1000 // ms
        shared.addBufferUpdateTime(duration)
    }
    
    private func addRenderTime(_ time: Double) {
        renderTimes.append(time)
        if renderTimes.count > maxSamples {
            renderTimes.removeFirst()
        }
        if renderTimes.count % 10 == 0 { // Log every 10 frames
            logStats()
        }
    }
    
    private func addUpdateTime(_ time: Double) {
        updateTimes.append(time)
        if updateTimes.count > maxSamples {
            updateTimes.removeFirst()
        }
    }
    
    private func addBufferUpdateTime(_ time: Double) {
        bufferUpdateTimes.append(time)
        if bufferUpdateTimes.count > maxSamples {
            bufferUpdateTimes.removeFirst()
        }
    }
    
    static func startDisplayTracking() {
        guard RenderingConstants.enablePerformanceLogging else { return }
        shared.startDisplayLink()
    }
    
    static func stopDisplayTracking() {
        shared.stopDisplayLink()
    }
    ...
    @objc private func displayLinkFired(_ displayLink: CADisplayLink) {
        ...
        let frameTime = (currentTime - lastFrameTime) * 1000 // ms
        
        // Detect frame drops (>20ms = dropped frame at 60fps)
        if frameTime > 20.0 {
            frameDropCount += 1
        }
        
        actualFrameTimes.append(frameTime)
        if actualFrameTimes.count > maxSamples {
            actualFrameTimes.removeFirst()
        }
    ...
    private func logStats() {
        let renderAvg = renderTimes.isEmpty ? 0 : renderTimes.reduce(0, +) / Double(renderTimes.count)
        let renderMax = renderTimes.max() ?? 0
        
        let updateAvg = updateTimes.isEmpty ? 0 : updateTimes.reduce(0, +) / Double(updateTimes.count)
        let bufferAvg = bufferUpdateTimes.isEmpty ? 0 : bufferUpdateTimes.reduce(0, +) / Double(bufferUpdateTimes.count)
        
        // Actual display performance
        let actualFrameAvg = actualFrameTimes.isEmpty ? 0 : actualFrameTimes.reduce(0, +) / Double(actualFrameTimes.count)
        let actualFPS = actualFrameAvg > 0 ? 1000.0 / actualFrameAvg : 0
        
        print("[PERF] Metal Render: \(String(format: "%.2f", renderAvg))ms avg, \(String(format: "%.2f", renderMax))ms max")
        print("[PERF] Display: \(String(format: "%.2f", actualFrameAvg))ms avg, \(String(format: "%.1f", actualFPS)) fps, \(frameDropCount) drops")
        
        if !updateTimes.isEmpty {
            print("[PERF] MetalData Update: \(String(format: "%.2f", updateAvg))ms avg")
        }
        if !bufferUpdateTimes.isEmpty {
            print("[PERF] Buffer Update: \(String(format: "%.2f", bufferAvg))ms avg")
        }
        
        // Reset frame drop counter periodically
        frameDropCount = 0
        
        // Force console output in playground
        fflush(stdout)
    }
}
...
class RenderingParams: ObservableObject {
    ...
    @Published var influenceThreshold: Float = 0.01 // Lower = higher quality, higher = better performance
...
class SplatterViewModel: ObservableObject {
    ...
    func updateMetalData() {
        let startTime = PerformanceMonitor.startUpdate()
        ...
        PerformanceMonitor.endUpdate(startTime)
    }
...
struct MetalOverlayView: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: MTKView, context: UIViewRepresentableContext<MetalOverlayView>) {
        ...
        coordinator.stateManager.updateRenderMask(renderMask)
        coordinator.stateManager.updateMetalDots(dots)
    ...
    class Coordinator: NSObject, MTKViewDelegate {
        ...
        func draw(in view: MTKView) {
            let startTime = PerformanceMonitor.startRender()
            ...
            MetalService.shared.renderer.render(
                drawable: drawable,
