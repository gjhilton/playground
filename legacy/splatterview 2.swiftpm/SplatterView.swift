...
class RenderingParams: ObservableObject {
    ...
    @Published var foregroundPassEnabled: Bool = false
    @Published var backgroundPassColor: Color = .black
    @Published var foregroundPassColor: Color = Color(red: 0.2, green: 0.2, blue: 0.8)
...
class SplatterViewModel: ObservableObject {
    ...
    private func computeRenderMask() -> UInt32 {
        var mask: UInt32 = 0
        if centralDot.enabled { mask |= RenderingConstants.centralDotMask }
        if largeDots.enabled { mask |= RenderingConstants.largeDotMask }
        if mediumDots.enabled { mask |= RenderingConstants.mediumDotMask }
        if smallDots.enabled { mask |= RenderingConstants.smallDotMask }
        return mask
    }
    ...
    func addSplat(at point: CGPoint, screenWidth: CGFloat, screenHeight: CGFloat) {
        ...
        if centralDot.enabled {
            let radius = rng.float(in: centralDot.radiusMin...centralDot.radiusMax)
        ...
        if largeDots.enabled {
            for _ in 0..<largeDots.count {
                ...
                let radius = rng.float(in: largeDots.radiusMin...largeDots.radiusMax)
        ...
        if mediumDots.enabled {
            for _ in 0..<mediumDots.count {
                ...
                let radius = rng.float(in: mediumDots.radiusMin...mediumDots.radiusMax)
        ...
        if smallDots.enabled {
            for _ in 0..<smallDots.count {
                ...
                let radius = rng.float(in: smallDots.radiusMin...smallDots.radiusMax)
...
private let colorPresets: [Color] = [
    ...
    .cyan, .blue, .indigo,
    .purple, .pink, .brown,
    Color(red: 0.2, green: 0.2, blue: 0.8), // Blue ink
    Color(red: 0.8, green: 0.2, blue: 0.2), // Red ink
    Color(red: 0.2, green: 0.6, blue: 0.2)  // Green ink
]
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 12) {
                        ...
                        VStack {
                            ...
                            if viewModel.rendering.backgroundPassEnabled {
                                VStack {
                                    ...
                                    // Color preset buttons (always available)
                                    LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 6), spacing: 8) {
                                        ForEach(colorPresets, id: \.self) { color in
                                            Button(action: {
                                                viewModel.rendering.backgroundPassColor = color
                                            }) {
                                                Rectangle()
                                                    .fill(color)
                                                    .frame(width: 30, height: 30)
                            ...
                            if viewModel.rendering.foregroundPassEnabled {
                                VStack {
                                    ...
                                    LazyVGrid(columns: Array(repeating: GridItem(.flexible()), count: 6), spacing: 8) {
                                        ForEach(colorPresets, id: \.self) { color in
                                            Button(action: {
                                                viewModel.rendering.foregroundPassColor = color
                                            }) {
                                                Rectangle()
                                                    .fill(color)
                                                    .frame(width: 30, height: 30)
                ...
                // Central Dot Controls
                GroupBox("Central Dot") {
                    VStack(spacing: 8) {
                        Toggle("Enabled", isOn: $viewModel.centralDot.enabled)
                        
                        if viewModel.centralDot.enabled {
                            VStack(spacing: 4) {
                                HStack {
                ...
                GroupBox("Large Dots") {
                    VStack(spacing: 8) {
                        Toggle("Enabled", isOn: $viewModel.largeDots.enabled)
                        
                        if viewModel.largeDots.enabled {
                            VStack(spacing: 4) {
                                HStack {
                ...
                // Medium Dots Controls
                GroupBox("Medium Dots") {
                    VStack(spacing: 8) {
                        Toggle("Enabled", isOn: $viewModel.mediumDots.enabled)
                        
                        if viewModel.mediumDots.enabled {
                            VStack(spacing: 4) {
                                HStack {
                ...
                GroupBox("Small Dots") {
                    VStack(spacing: 8) {
                        Toggle("Enabled", isOn: $viewModel.smallDots.enabled)
                        
                        if viewModel.smallDots.enabled {
                            VStack(spacing: 4) {
                                HStack {
