    ...
    var body: some View {
        GeometryReader { geo in
            if RenderingConstants.showParameterControls {
                ...
                // Production mode - clean view only
                ZStack {
                    ForEach(viewModel.renderPasses, id: \.name) { renderPass in
                        if renderPass.enabled {
                            MetalOverlayView(
    ...
    var body: some View {
        ScrollView {
            VStack(spacing: 10) {
                ...
                GroupBox("Dot Types") {
                    VStack {
                        Toggle("Central Dot", isOn: $viewModel.centralDot.enabled)
                        Toggle("Large Dots", isOn: $viewModel.largeDots.enabled)
                        Toggle("Medium Dots", isOn: $viewModel.mediumDots.enabled)
                        Toggle("Small Dots", isOn: $viewModel.smallDots.enabled)
                    }
                }
            }
            .padding()
