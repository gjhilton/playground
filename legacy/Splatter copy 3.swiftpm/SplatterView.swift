...
class RenderingParams: ObservableObject {
    ...
    var backgroundColorSIMD: SIMD3<Float> {
        let uiColor = UIColor(backgroundPassColor)
    ...
    var foregroundColorSIMD: SIMD3<Float> {
        let uiColor = UIColor(foregroundPassColor)
        var r: CGFloat = 0, g: CGFloat = 0, b: CGFloat = 0, a: CGFloat = 0
        uiColor.getRed(&r, green: &g, blue: &b, alpha: &a)
        return SIMD3<Float>(Float(r), Float(g), Float(b))
    }
...
struct SplatterView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ZStack {
                // Background Pass - All dot types (drawn first)
                if viewModel.rendering.backgroundPassEnabled {
                    MetalOverlayView(xs: viewModel.dotXs, ys: viewModel.dotYs, radii: viewModel.dotRadii, types: viewModel.dotTypes, splotColor: viewModel.backgroundColorSIMD, renderMask: [true, true, true, true, false]) // central, large, medium, small, no splash
                        .ignoresSafeArea()
                        .allowsHitTesting(false)
                        .blendMode(.multiply)
                        .zIndex(1) // Ensure background renders first
                }
                
                // Foreground Pass - Only central dots (drawn second, on top)
                if viewModel.rendering.foregroundPassEnabled {
                    MetalOverlayView(xs: viewModel.dotXs, ys: viewModel.dotYs, radii: viewModel.dotRadii, types: viewModel.dotTypes, splotColor: viewModel.foregroundColorSIMD, renderMask: [true, false, false, false, false]) // only central
                        .ignoresSafeArea()
                        .allowsHitTesting(false)
                        .blendMode(.multiply)
                        .zIndex(2) // Ensure foreground renders on top
                }
            }
            ...
            .onChange(of: viewModel.rendering.backgroundPassEnabled) {
                print("🔄 SplatterView.onChange(of: backgroundPassEnabled) triggered")
                viewModel.updateMetalData()
            }
            .onChange(of: viewModel.rendering.foregroundPassEnabled) {
                print("🔄 SplatterView.onChange(of: foregroundPassEnabled) triggered")
                viewModel.updateMetalData()
            }
            .onChange(of: viewModel.rendering.backgroundPassColor) {
                print("🔄 SplatterView.onChange(of: backgroundPassColor) triggered")
                viewModel.updateMetalData()
            }
            .onChange(of: viewModel.rendering.foregroundPassColor) {
                print("🔄 SplatterView.onChange(of: foregroundPassColor) triggered")
                viewModel.updateMetalData()
            }
