...
class SplatterView: ObservableObject {
    ...
    func clear() {
        print("🧽 SplatterView.clear() called")
        splats.removeAll()
        print("🧽 SplatterView.clear() - splats cleared, count now: \(splats.count)")
    }
    
    func touched(center: CGPoint) {
        print("👆 SplatterView.touched(center:) called at \(center)")
        splats.append(Splat.generate(center: center))
        print("👆 SplatterView.touched(center:) - splat added, count now: \(splats.count)")
    }
    
    func updateMetalData(width: CGFloat, height: CGFloat) {
        print("🔧 SplatterView.updateMetalData() called with size: \(width) x \(height)")
        ...
        print("🔧 SplatterView.updateMetalData() - Metal data updated")
    }
}

struct SplatterViewUI: View {
    @ObservedObject var splatterView: SplatterView
    
    var body: some View {
        GeometryReader { geo in
            MetalOverlayView(xs: splatterView.overlayDotXs, ys: splatterView.overlayDotYs, radii: splatterView.overlayDotRadii, splotColor: splatterView.splotColor)
                .ignoresSafeArea()
                .allowsHitTesting(false)
                .blendMode(.multiply)
                .onChange(of: splatterView.splats) {
                    print("🔄 SplatterViewUI.onChange(of: splats) triggered")
                    splatterView.updateMetalData(width: geo.size.width, height: geo.size.height)
                }
        }
    }
}
