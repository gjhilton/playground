...
/// Debug-only input validation that's removed in release builds for zero performance impact
struct InputValidator {
    ...
    static func validateDotPosition(_ position: CGPoint, context: String = "") -> CGPoint {
        ...
        if position.x != clampedX || position.y != clampedY {
            print("⚠️ InputValidator: Clamped position from (\(position.x), \(position.y)) to (\(clampedX), \(clampedY)) \(context)")
        }
    ...
    static func validateRadius(_ radius: Float, context: String = "") -> Float {
        let clampedRadius = max(0, min(1, radius))
        
        if radius != clampedRadius {
            print("⚠️ InputValidator: Clamped radius from \(radius) to \(clampedRadius) \(context)")
        }
    ...
    static func validateDotType(_ type: Int32, context: String = "") -> Int32 {
        let clampedType = max(0, min(Int32(RenderingConstants.dotTypeCount - 1), type))
        
        if type != clampedType {
            print("⚠️ InputValidator: Clamped dot type from \(type) to \(clampedType) \(context)")
        }
    ...
    static func validateRenderMask(_ mask: [Bool], context: String = "") -> [Bool] {
        let expectedCount = RenderingConstants.dotTypeCount
        if mask.count != expectedCount {
            print("⚠️ InputValidator: Render mask has \(mask.count) elements, expected \(expectedCount) \(context)")
            return Array(mask.prefix(expectedCount)) + Array(repeating: false, count: max(0, expectedCount - mask.count))
        }
        return mask
    }
    #else
    // Release builds: validation is completely removed, zero performance impact
    @inline(__always)
    static func validateDotPosition(_ position: CGPoint, context: String = "") -> CGPoint { position }
    
    @inline(__always)
    static func validateRadius(_ radius: Float, context: String = "") -> Float { radius }
    
    @inline(__always)
    static func validateDotType(_ type: Int32, context: String = "") -> Int32 { type }
    
    @inline(__always)
    static func validateRenderMask(_ mask: [Bool], context: String = "") -> [Bool] { mask }
    #endif
}
