...
struct MetalDotData {
    ...
    init(from dots: [SplatDot], limit: Int) {
        ...
        self.ys = limitedDots.map { 1 - Float($0.position.y) } + Array(repeating: 0, count: paddingCount)
...
class SplatterViewModel: ObservableObject {
    ...
    private func setupReactiveUpdates() {
        // Combine all change triggers into a single reactive stream
        Publishers.CombineLatest4(
            ...
            Publishers.CombineLatest(
                ...
                rendering.$foregroundPassEnabled
            ),
            Publishers.CombineLatest(
                rendering.$backgroundPassColor,
                rendering.$foregroundPassColor
            )
        )
        .debounce(for: .milliseconds(RenderingConstants.reactiveUpdateDebounceMs), scheduler: RunLoop.main) // Debounce to avoid excessive updates
