...
struct MetalOverlayView: UIViewRepresentable {
    ...
    func updateUIView(_ uiView: MTKView, context: UIViewRepresentableContext<MetalOverlayView>) {
        ...
        // Update unified data (used by Metal shader) - fast hash comparison
        let newHash = dots.hashValue
        if coordinator.metalDotsHash != newHash {
            coordinator.metalDots = dots
            coordinator.metalDotsHash = newHash
        }
    ...
    class Coordinator: NSObject, MTKViewDelegate {
        ...
        // Performance optimization: lightweight hash for change detection
        var metalDotsHash: Int = 0
        
        // Cache render mask state to avoid redundant GPU updates
