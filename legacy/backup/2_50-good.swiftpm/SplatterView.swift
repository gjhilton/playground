...
struct MetalOverlayView: UIViewRepresentable {
    ...
    class Coordinator: NSObject, MTKViewDelegate {
        ...
        private var xsBuffer: MTLBuffer?
        private var ysBuffer: MTLBuffer?
        private var radiiBuffer: MTLBuffer?
        private var typesBuffer: MTLBuffer?
        ...
        private func createBuffers() {
            ...
            xsBuffer = device.makeBuffer(length: floatSize * RenderingConstants.splatDotLimit, options: [.storageModeShared])
            ysBuffer = device.makeBuffer(length: floatSize * RenderingConstants.splatDotLimit, options: [.storageModeShared])
            radiiBuffer = device.makeBuffer(length: floatSize * RenderingConstants.splatDotLimit, options: [.storageModeShared])
            typesBuffer = device.makeBuffer(length: intSize * RenderingConstants.splatDotLimit, options: [.storageModeShared])
        ...
        private func updateBuffers() {
            // Update xs buffer
            if let buffer = xsBuffer {
                let pointer = buffer.contents().bindMemory(to: Float.self, capacity: RenderingConstants.splatDotLimit)
            ...
            // Update types buffer
            if let buffer = typesBuffer {
                let pointer = buffer.contents().bindMemory(to: Int.self, capacity: RenderingConstants.splatDotLimit)
                for i in 0..<RenderingConstants.splatDotLimit {
                    pointer[i] = types[i]
                }
            }
        ...
        func draw(in view: MTKView) {
            guard let drawable = view.currentDrawable,
                  ...
                  let xsBuffer = xsBuffer,
                  let ysBuffer = ysBuffer,
                  let radiiBuffer = radiiBuffer,
                  let typesBuffer = typesBuffer,
            ...
            // Use MTLBuffer objects instead of setFragmentBytes for large arrays
            encoder.setFragmentBuffer(xsBuffer, offset: 0, index: 1)
            encoder.setFragmentBuffer(ysBuffer, offset: 0, index: 2)
            encoder.setFragmentBuffer(radiiBuffer, offset: 0, index: 3)
            encoder.setFragmentBuffer(typesBuffer, offset: 0, index: 4)
            ...
            encoder.setFragmentBuffer(renderMaskBuffer, offset: 0, index: 7)
